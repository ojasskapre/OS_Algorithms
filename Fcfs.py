...
def gantt_chart(process_list):
    ...
    while True:
        ...
        for items in process_list:
            if time == items['arrival']:
                active_process.append((items['arrival'], items['PId'], items['burst']))
        time = time + 1
    ...
    while len(temp) != 0:
        for items in temp:
            ...
            if time >= items[0]:
                time = time + items[2]
                table.append({'PID': items[1], 'AT': items[0], 'BT': items[2], 'CT': time, 'TAT': time - items[0],
                              'WT': time - items[0] - items[2]})
                turn_around_time.append(time - items[0])
