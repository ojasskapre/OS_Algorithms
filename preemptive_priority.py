    ...
    while len(active_process) > 0 or len(process_list) > 0:
        ...
        while j < len(process_list):
            if time == process_list[j]['arrival']:
                active_process.append({'PId': process_list[j]['PId'], 'at': process_list[j]['arrival'], 'bt': process_list[j]['burst'], 'priority': process_list[j]['priority'], 'rt': process_list[j]['burst']})
                del process_list[j]
                j = j - 1
            j = j + 1
        ...
        if time >= end and len(active_process) > 0:
            priority = active_process[0]['priority']
            position = 0
            for itr in range(len(active_process)):
                if active_process[itr]['at'] <= time:
                    if active_process[itr]['priority'] < priority:
                        priority = active_process[itr]['priority']
                        position = itr
                    elif active_process[itr]['priority'] == priority:
